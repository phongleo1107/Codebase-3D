...
- [ ] `app/config.py` — `Settings` + all limit constants
- [ ] `app/errors.py` + `app/models/` + `app/logging_setup.py` — freeze the API contract so frontend work can start in parallel
...
- [ ] `app/api/` — routes, body-size middleware, rate limiter, concurrency gate, error handlers
