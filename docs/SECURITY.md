...
> **Many security controls in this document are still not implemented.** As of 2026-08-29 the backend contract layer (config, errors, models, logging), the **URL/egress security boundary** (`app/security/url_validation.py`, `app/security/net_guard.py`), the **GitHub client that calls it** (`app/fetch/github.py`), the **streaming archive reader** (`app/fetch/archive.py`), and the **import extractor** (`app/analysis/parser.py`) exist. Two further security modules — `app/security/secret_filter.py` and `app/security/path_safety.py` — are implemented and tested but **have no callers at all**; a rule nothing applies protects nothing, which is why the row each one backs is not simply `Implemented`. There is still no routing and no analysis pipeline, and **nothing in this codebase has ever sent the download request** — `github.py` resolves and builds it, `archive.py` consumes the bytes it would return, `parser.py` extracts imports from bytes nothing has yet handed it, and no caller joins any of the three. The archive and parser controls below are therefore implemented and tested against in-process fixtures, not against a real repository. Rows marked `Implemented` name the file; treat every other row as aspirational. Flip a row only when you have seen the code, and name the file in the same edit.
>
