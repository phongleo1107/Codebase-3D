...
Graph Model           (GraphNode[] / GraphEdge[] / Stats / ServiceMap)
 ↓
Component diagram     (deterministic Mermaid, derived from the finished graph — ADR-013, ADR-023)
 ↓
...
### Component diagram · *Implemented* — `app/analysis/component_diagram.py`, ADR-013, ADR-023
...
Three things become three kinds of box, as ADR-013 specifies. **Containers are top-level directories**, derived from *file node paths* rather than from directory nodes — first path component, plus a synthetic `(root)` container for files sitting directly in the repository — so ADR-018's file/directory collision cannot silently delete one. **External packages are one box with a total**, `External packages · N imports`, because ADR-005 kept package *names* out of the graph entirely and this module is deliberately not given the resolver's specifiers (ADR-023). **Detected routes are the API surface**, one box per endpoint labelled `METHOD /path` plus the handler's own comment where one exists, each pointing at the container that declares it. Import edges are aggregated onto container pairs with the collapsed count on the arrow; edges inside one container are dropped.
