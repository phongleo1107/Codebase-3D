...
| `app/api/` | Routes (`analyze`, `health`), middleware, rate limiter, concurrency gate | **In progress** — `routes.py` (`POST /api/analyze`, `GET /api/health`), `middleware.py` (`RequestIdMiddleware`, `BodySizeLimitMiddleware`), `app.py` (the factory plus four exception handlers), and `app/main.py` as the ASGI entry point. Rate limiter, concurrency gate, and CORS are Day 3 (ADR-008, ADR-011); `/api/source` is deferred post-MVP (ADR-007, ADR-013) |
...
|---|---|---|
| `src/api/` | Client + zod schemas; validates and caps every response | *Schema implemented* (`schema.ts`, `limits.ts`). **No client yet** — `POST /api/analyze` exists now, so this is unblocked and simply unwritten |
| `src/graph/` | Normalization, adjacency, collapse/expand transform (Cytoscape compound nodes), search | *Partial* — `elements.ts` (wire graph → Cytoscape elements) and `fixture.ts` (a hand-written response standing in for the server). No adjacency index, no collapse/expand action, no search |
...
| `src/ui/` | Landing, loading, tooltip, inspector (description, imports/importedBy), component diagram panel (`mermaid`), service map panel, status bar. Search/tree panel and source preview are deferred post-MVP | *Partial* — `App.tsx` (shell + status bar), `Inspector.tsx`, and `ComponentDiagram.tsx`. No landing page, no loading state, **no service-map panel**. The diagram panel exists and `mermaid` is pinned at 11.17.2 (ADR-025), but **nothing imports the panel** — it is not in `App.tsx`'s layout, so the running app shows only the canvas and the inspector. *(Corrected 2026-09-01: this cell used to read "no diagram or service-map panel; `mermaid` is not a dependency yet".)* |
| `src/store/` | Zustand store | *Implemented* — `graphStore.ts`: the response and the selected node id, nothing derived |

**Nothing here has been driven by a real response.** The canvas renders `src/graph/fixture.ts` because the frontend has no API client — not because there is nothing to call: `POST /api/analyze` has existed since ADR-023 and returns a full response including `componentDiagram`. `AnalyzeResponseSchema` has therefore never validated server output, only a fixture written to satisfy it, and writing the client is what closes that gap. This is still true of the diagram panel added on 2026-09-01: it was driven from the fixture, in a real browser, but never from the server. What is *no longer* true is the stronger claim this document and SECURITY.md used to make — that no frontend claim rested on an observed render. Some now do, for one panel, manually; see below.
...
3. **API → Browser** — zod validation with hard caps (`frontend/src/api/schema.ts`, *Implemented*, but **never yet run against a server response** — only against `src/graph/fixture.ts`); repository text rendered as text nodes, never as an HTML string (`frontend/src/ui/Inspector.tsx`, *Implemented for `description`*; `summary` still has no sink). The diagram is the one sink where that rule cannot apply, because mermaid hands back markup: `frontend/src/ui/ComponentDiagram.tsx` is *Implemented* under ADR-025 — adopted from an inert `DOMParser` document behind an element allowlist, with no `dangerouslySetInnerHTML` or `innerHTML` anywhere in `src/` — and is **not yet imported by anything**, so no repository text reaches it in the running app. `MAX_NODES` / `MAX_EDGES` are enforced on the server first (ADR-023), so the browser cap is now ordinary defence in depth rather than the only depth — but the two are kept in step **by hand**, and `limits.ts` drifting above `Settings` turns a legal response into a blank screen, since zod rejects the whole document rather than trimming it.
