...
|---|---|---|
| `src/api/` | Client + zod schemas; validates and caps every response | *Schema implemented* (`schema.ts`, `limits.ts`). **No client** — there is no endpoint to call |
...
| `src/ui/` | Landing, loading, tooltip, inspector (description, imports/importedBy), component diagram panel (`mermaid`), service map panel, status bar. Search/tree panel and source preview are deferred post-MVP | *Partial* — `App.tsx` (shell + status bar) and `Inspector.tsx`. No landing page, no loading state, no diagram or service-map panel; `mermaid` is not a dependency yet |
| `src/store/` | Zustand store | *Implemented* — `graphStore.ts`: the response and the selected node id, nothing derived |

**Nothing here has been driven by a real response.** The canvas renders `src/graph/fixture.ts`, because `POST /api/analyze` does not exist. `AnalyzeResponseSchema` has therefore never validated server output, only a fixture written to satisfy it.
...
|---|---|---|
| `POST /api/analyze` | `{repository_url}` → `{repository, nodes, edges, stats, serviceMap, componentDiagram}` | **Implemented** — `app/api/routes.analyze`. `componentDiagram` is `None` until its generator exists |
