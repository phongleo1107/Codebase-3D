...
The two unwired panels are the exception, and the distinction matters for what their evidence is worth. `ComponentDiagram.tsx` was driven in a real browser, but **from `fixture.ts`, never from the server** — so the ADR-025 observations below rest on a hand-written diagram, not on one the generator produced end to end. `ServiceMap.tsx` has not been rendered at all. What is *no longer* true is the stronger claim this document and SECURITY.md used to make — that no frontend claim rested on an observed render. Some now do, for one panel, manually and unrepeatably; see below.
