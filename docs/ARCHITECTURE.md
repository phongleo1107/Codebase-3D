...
>
> **2026-09-02 — CORS is implemented, as an explicit origin allowlist (ADR-028).** `app/config.py` gained `CORS_ALLOWED_ORIGINS` (default empty = nothing allowed cross-origin), enforced by `CORSMiddleware` as the **outermost** middleware in `app/api/app.py`, so a preflight is answered without the application (or the request-id/body-cap middleware) running. Methods are limited to GET/POST/OPTIONS, headers to `Content-Type`, credentials are off, and there is no wildcard or reflected `Origin`. The deploy scaffolding landed with it: `frontend/vercel.json`, `render.yaml`, `.env.example`, and `.vercel/` in `.gitignore`.
>
...
| `app/api/` | Routes (`analyze`, `health`), middleware, rate limiter, concurrency gate, CORS allowlist | **Implemented for MVP scope** — `routes.py` (`POST /api/analyze`, `GET /api/health`), `middleware.py` (`RequestIdMiddleware`, `BodySizeLimitMiddleware`), `rate_limit.py` (`SlidingWindowLimiter`, `ConcurrencyGate`, ADR-008), `app.py` (the factory, the four exception handlers, and the outermost `CORSMiddleware` — allowlist from `Settings.CORS_ALLOWED_ORIGINS`, ADR-028), and `app/main.py` as the ASGI entry point. `/api/source` is deferred post-MVP (ADR-007, ADR-013) |
...
React 19, TypeScript 5.9.3, Vite 8, Cytoscape.js 3.34, Tailwind v4, Zustand 5, zod 4, and `mermaid` 11.17.2 — all pinned exactly in `frontend/package.json`, with `package-lock.json` committed. `mermaid` (added 2026-09-01) is the largest of them by a wide margin: 159 transitive packages, including a second copy of `cytoscape`. *(2026-09-01, ADR-022: was Three.js/React Three Fiber; see the note at the top of this document.)*
...
| `src/ui/` | Landing, loading, tooltip, inspector (description, imports/importedBy), component diagram panel (`mermaid`), service map panel, status bar. Search/tree panel and source preview are deferred post-MVP | *Partial* — `App.tsx` (landing page + URL input + loading/error state + shell + status bar, **done 2026-09-01**), `Inspector.tsx`, `ComponentDiagram.tsx`, and `ServiceMap.tsx`. All four MVP pieces now exist, but the last two are **written and unwired** — nothing imports either, so the running app shows only the canvas and the inspector. `mermaid` is pinned at 11.17.2 (ADR-025). `ComponentDiagram.tsx` is the **only** file here with a test (`ComponentDiagram.test.tsx`); see Testing below. *(Corrected 2026-09-01: this cell used to read "no diagram or service-map panel; `mermaid` is not a dependency yet".)* |
...
**The frontend has now been driven by a real response — but only the half of it that is wired up.** `src/api/client.ts` posts to `/api/analyze` and validates the result with `AnalyzeResponseSchema` before anything else touches it — the first thing that has run that schema against server output rather than the hand-written `src/graph/fixture.ts`. Run through the Vite dev server (proxied to the backend at `/api`, so the browser's request stays same-origin) against three real repositories, the schema validated the response unmodified. *(Corrected 2026-09-02: the parenthetical used to say \"since the backend has no CORS yet — Day 3\". CORS is now implemented (ADR-028); the proxy still exists because a same-origin relative `/api` avoids the preflight entirely, and in production `VITE_API_URL` plus the backend's allowlist replace it.)* The canvas now renders whatever `App.tsx`'s landing-page form submits, not a fixture.

The two unwired panels are the exception, and the distinction matters for what their evidence is worth. `ComponentDiagram.tsx` was driven in a real browser and is now **also covered by an automated suite**, but in both cases **from `fixture.ts`, never from the server** — so the ADR-025 observations below rest on a hand-written diagram, not on one the generator produced end to end. `ServiceMap.tsx` has not been rendered at all and has no test. What is *no longer* true is the stronger claim this document and SECURITY.md used to make — that no frontend claim rested on an observed render. Some now do, for one panel. *(Corrected 2026-09-01: this used to end "manually and unrepeatably". The suite below made them repeatable — under jsdom, not a browser.)*

**Testing · *Implemented for one panel; absent everywhere else*** — `frontend/vitest.config.ts`, **vitest 4.1.11 + jsdom 30.0.1, 35 tests, `npm test`**. Two files: `src/ui/ComponentDiagram.test.tsx` automates ADR-025's interactive session, and `src/no-markup-sinks.test.ts` asserts that no file in `src/` uses `dangerouslySetInnerHTML` / `innerHTML` / `outerHTML` / `insertAdjacentHTML` / `document.write`. Only the second of those covers the app broadly; `App.tsx`, `GraphCanvas.tsx`, `Inspector.tsx`, `ServiceMap.tsx`, the store and the client have **no tests of their own**. And the suite is **jsdom, not a browser** — it needs two SVG-metric stubs (`getComputedTextLength`, `getBBox`) and one row patched into parse5's SVG tag-name adjustment table to run at all, so its assertions are about jsdom's parser and DOMPurify under it rather than Chrome's. ADR-025's Correction records both, and why the parse5 gap would otherwise make a correct component look broken.

### Rendering · *Implemented* — `src/scene/GraphCanvas.tsx`

Cytoscape.js (ADR-022, supersedes ADR-002's custom R3F scene). Directory hierarchy still maps onto Cytoscape's compound nodes via `GraphNode.parent` (ADR-006) → `data.parent` with no transform beyond a rename, but as of ADR-026 that is a layout hook only — compound nodes render with zero background opacity and zero border width, not as visible boxes. *(Corrected 2026-09-01: this used to say compound nodes are "what backs collapse/expand," implying a visible nested-box hierarchy. Hands-on testing against a real repository (`expressjs/express`, 141 files/153 import edges) showed that rendering as up to 5 levels of nested, padded boxes that reduced files to near-invisible dots at the center — directories dominated the visual and imports did not drive clustering. ADR-026 demotes directory rendering to a per-file color tint plus an external legend; `parent` still flows through for layout grouping and remains available for a future collapse/expand, but is no longer drawn.)*

Selection is three classes applied imperatively through a ref — `selected`, `neighbor` (a direct dependency or dependent, plus the enclosing directories so a selection never fades its own box), `faded` — so changing the selection re-runs neither the React subtree nor the layout. That is PRD §7's trace with no extra machinery. This logic was verified unchanged by ADR-026's rendering change.

One non-obvious constraint: the layout is **not** passed to the `cytoscape()` constructor. React commits the canvas subtree and runs the effect before the flex chain above it has a resolved height, so Cytoscape would size its canvases to 0×0 and lay out against an empty viewport. A `ResizeObserver` runs the layout on the first callback with a real box, and only refits afterwards. *(Corrected 2026-09-01: the refit half of this had a real bug — `cy.resize()`/`cy.fit()` ran synchronously against a `clientWidth`/`clientHeight` read that could still reflect the pre-resize box, leaving the graph stuck shrunk in its original bounding box after a later viewport resize. Fixed by reading the observer entry's `contentRect`, deferring the fit to `requestAnimationFrame`, and adding a bounded re-check. See ADR-026's Consequences.)*

### Layout · *Implemented* — `src/scene/style.ts`

**`cytoscape-fcose`** (ADR-026, replacing the `cose` placeholder this section used to describe as still open between `cola`/`elk`/`dagre`). Chosen after that measurement finally happened — against a real repository, not a ten-node fixture — because it treats compound nesting as a soft constraint rather than the dominant layout signal, and scales toward this project's real ceiling (`MAX_NODES=6000`/`MAX_EDGES=20000`) rather than a demo-sized graph. Runs on the main thread since the graph is bounded by `MAX_NODES`/`MAX_EDGES`; no custom worker or force-refinement pass (ADR-004, superseded by ADR-022).

### Component diagram and service map · *Diagram implemented and not wired in; service map planned*

The two statuses diverged on 2026-09-01, so read the halves separately. The section keeps its combined title because "Component diagram" already names the *backend* generator above.

**The diagram panel — *Implemented, not wired in*: `frontend/src/ui/ComponentDiagram.tsx`, ADR-025.** Rendered client-side with `mermaid` 11.17.2 (exact-pinned), fed the `componentDiagram` field's Mermaid source verbatim from `/api/analyze`. The backend half is **Implemented** (ADR-024, see "Component diagram" above). The panel is written, configured, driven in a real browser, and — since later the same day — covered by an automated jsdom suite; but **nothing imports it**, so it is absent from `App.tsx`'s layout and from the app as it runs. That is the render decided and the layout not.

This is the project's only sink where repository text arrives back as **markup** rather than sitting beside it, and `mermaid.render()` returns an SVG *string*, so the defence is four layers deep: ADR-024 removes the injection site in the generator (synthetic ids, so repository text occupies only a quoted label), mermaid DOMPurifies each label, mermaid DOMPurifies the serialized SVG because `securityLevel` is not `loose`, and then this file — the only layer we own — **adopts rather than injects** (ADR-025): parse the string as `image/svg+xml` into an inert document, refuse on an XML parse error or a non-SVG root, walk the inert tree and refuse any element outside a 15-element observed allowlist or any `on*` / `href` / `*:href` / `base` attribute, and only then `document.importNode`. Refuse, not scrub. `htmlLabels: false` is a control and not styling — with it on, labels become HTML inside a `foreignObject` and mermaid *widens* DOMPurify to keep them. The full argument, the mermaid source it was read from, and the observations behind the allowlist live in ADR-025 and in the file's own header; CURRENT_STATE.md records what was measured. Two caveats belong here rather than there. First, the verification is **automated but not a browser**: `frontend/src/ui/ComponentDiagram.test.tsx` (vitest 4.1.11 + jsdom 30.0.1, part of the 35-test frontend suite) re-runs the allowlist, the config controls and every refusal path on each `npm test`, which is what makes the allowlist a regression-detector rather than a memory — but it runs under jsdom, on two SVG-metric stubs and one patched parse5 row, so it is not evidence about Chrome. *(Corrected 2026-09-01: this sentence used to read "the verification was **manual**, `frontend/` has **no tests**". Both halves are now false. The suite also corrected ADR-025 on which layer refuses a `click … href` directive — see that ADR's Correction.)* Second, `<style>` is on the allowlist with its residual (`url()` exfiltration, closed by the planned CSP) recorded rather than closed.

**The service map — *Planned*. It has no panel at all.** It is planned as a simple grouped list (route → file → summary, where a summary exists), sourced from the `serviceMap` field, read-only display of server-computed text. Until it exists, `ServiceEndpoint.summary` has **no sink of its own** — the diagram embeds route summaries inside labels, which is a different thing from rendering the field.
...
3. **API → Browser** — zod validation with hard caps (`frontend/src/api/schema.ts`, *Implemented*, and **run against a real server response as of 2026-09-01** via `frontend/src/api/client.ts` — three real repositories, no field needed to change); repository text rendered as text nodes, never as an HTML string (`frontend/src/ui/Inspector.tsx`, *Implemented for `description`*; `frontend/src/ui/ServiceMap.tsx`, *Implemented for `summary`* on a code-path reading only — that panel's DOM has never been inspected). The diagram is the one sink where that rule cannot apply, because mermaid hands back markup: `frontend/src/ui/ComponentDiagram.tsx` is *Implemented* under ADR-025 — adopted from an inert `DOMParser` document behind an element allowlist, with no `dangerouslySetInnerHTML` or `innerHTML` anywhere in `src/`. **Neither the diagram panel nor the service map is imported by anything**, so no repository text reaches either in the running app. `MAX_NODES` / `MAX_EDGES` are enforced on the server first (ADR-023), so the browser cap is now ordinary defence in depth rather than the only depth — but the two are kept in step **by hand**, and `limits.ts` drifting above `Settings` turns a legal response into a blank screen, since zod rejects the whole document rather than trimming it.
...
## Deployment · *In progress* (ADR-011, ADR-028)

Frontend (Vite build) deploys to **Vercel**; backend (FastAPI) deploys to a separate host that runs it as a persistent process — **Render** is the documented default, chosen in ADR-028, with Railway and Fly as equivalent alternatives — not Vercel's serverless Python runtime, which is a poor fit for a 60s analysis budget plus a streaming download plus a tree-sitter parse, and where the native grammar wheels may not load at all. `docker compose up` as a single-command local/self-hosted run remains the longer-term target (ADR-001) and should still be built once the split deployment is live and there is time to verify it.

The mechanics are in the tree: `frontend/vercel.json` pins the Vite framework, `npm run build`, and `dist`; `render.yaml` pins root directory `backend`, a `uv`-based build, and the `uvicorn` start command; `CORS_ALLOWED_ORIGINS` (backend) is the exact allowlist the deployment fills in; `VITE_API_URL` (frontend, build-time) points the browser at the backend. What is open is the live step: create the two services, set those variables, and verify one cross-origin `POST /api/analyze` against the production pair (docs/TODO.md, Day 3).
